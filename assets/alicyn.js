  ...
  function setupAjaxCartForms() {
    ...
    forms.forEach(function (form) {
      ...
      form.addEventListener("submit", function (event) {
        ...
        var endpoint = action.indexOf(".js") > -1 ? action : action.replace(/\/cart\/add\/?$/, "/cart/add.js");
        ...
        if (status) {
          status.textContent = "Agregando al carrito...";
        }
        ...
        fetch(endpoint, {
          ...
          .then(function (response) {
            if (!response.ok) {
              return response.json().catch(function () {
                ...
                throw new Error(payload.description || "No se pudo agregar al carrito.");
              });
            }
          ...
          .then(function () {
            var actionType = form.getAttribute("data-cart-action");
            if (button) {
              button.textContent = "Agregado";
            }
            if (status) {
              status.innerHTML = actionType === "checkout"
                ? 'Agregado. <a href="/checkout">Finalizar compra</a>'
                : 'Agregado. <a href="/cart">Ver carrito</a>';
            }

            document.dispatchEvent(new CustomEvent("alicyn:cart:add", {
              bubbles: true,
              detail: {
                form: form
              }
          ...
          .catch(function (error) {
            if (status) {
              status.textContent = error.message || "No se pudo agregar. Intenta desde el producto.";
            }
            if (button) {
